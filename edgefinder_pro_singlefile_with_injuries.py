# edgefinder_pro_singlefile_with_injuries.py
import os
...
st.set_page_config(page_title="EdgeFinder — Pro (Single File with Injuries/Lineups)", layout="wide")

# ---------------------------------------------------------------------
# Keys (required for "full pro" experience)
# ---------------------------------------------------------------------
...
APISPORTS_KEY   = require_key("APISPORTS_KEY")   # single key used across API-SPORTS products
...
# API-SPORTS base URLs per sport product
APISPORTS_BASE = {
    "Soccer":             "https://v3.football.api-sports.io",        # v3
    "Basketball":         "https://v1.basketball.api-sports.io",      # v1
    "Ice Hockey":         "https://v1.hockey.api-sports.io",          # v1
    "Baseball":           "https://v1.baseball.api-sports.io",        # v1
    "American Football":  "https://v1.american-football.api-sports.io" # v1
}

def apisports_get(group: str, path: str, **params):
    base = APISPORTS_BASE.get(group)
    if not base:
        raise RuntimeError(f"APISPORTS does not support group {group}")
    headers = {"x-apisports-key": APISPORTS_KEY}
    r = requests.get(base + path, headers=headers, params=params, timeout=25)
    if r.status_code != 200:
        # Return empty but log to UI for visibility
        try:
            err = r.json()
        except Exception:
            err = {"message": r.text}
        st.write(f"API-SPORTS warning {r.status_code} on {path}: {str(err)[:150]}")
        return {"response": []}
    return r.json()
...
# ---------------------------------------------------------------------
# Scores-derived features (no extra keys)
# ---------------------------------------------------------------------
...
# ---------------------------------------------------------------------
# APISPORTS helpers (injuries & lineups)
# ---------------------------------------------------------------------
def apisports_team_id(group: str, team_name: str):
    """Find a team ID by fuzzy search; returns first match id or None."""
    try:
        # soccer uses /teams ; others use /teams too
        resp = apisports_get(group, "/teams", search=team_name)
        arr = resp.get("response", [])
        if not arr:
            return None
        item = arr[0]
        # structure differs by product; try common shapes
        tid = (item.get("team") or {}).get("id") or item.get("id") or item.get("team_id")
        return tid
    except Exception:
        return None

def apisports_injuries(group: str, team_id: int, date_iso: str):
    """Return list of injury strings for a team around the given date (best effort)."""
    try:
        ymd = (date_iso or "")[:10]
        if group == "Soccer":
            data = apisports_get(group, "/injuries", team=team_id, date=ymd)
            out = []
            for row in data.get("response", []):
                p = row.get("player", {}) or {}
                i = row.get("injury", {}) or {}
                name = p.get("name") or "Player"
                r = i.get("reason") or i.get("type") or "Undisclosed"
                out.append(f"{name} — {r}")
            return out
        else:
            # Many v1 products: attempt /injuries?team=
            data = apisports_get(group, "/injuries", team=team_id)
            out = []
            for row in data.get("response", []):
                p = row.get("player", {}) or {}
                i = row.get("injury", {}) or {}
...
def apisports_lineup(group: str, team_id: int, date_iso: str):
    """Return list of probable starters (best effort)."""
    try:
        ymd = (date_iso or "")[:10]
        if group == "Soccer":
            # find fixture for date + team
            fx = apisports_get(group, "/fixtures", team=team_id, date=ymd).get("response", [])
            if not fx:
                return []
            fixture_id = (fx[0].get("fixture") or {}).get("id") or fx[0].get("id")
            if not fixture_id:
                return []
            line = apisports_get(group, "/fixtures/lineups", fixture=fixture_id).get("response", [])
            if not line:
                return []
            # choose team lineup matching our team_id
            squad = None
            for l in line:
                t = (l.get("team") or {}).get("id")
                if t == team_id:
                    squad = l
                    break
            if not squad:
                squad = line[0]
            starters = [(p.get("player") or {}).get("name") for p in squad.get("startXI", [])]
            starters = [s for s in starters if s]
            return starters[:11]
        else:
            # Not all v1 products expose lineups; best effort via /teams or /players not reliable
            return []
    except Exception:
        return []

def injuries_and_lineups(group: str, team_name: str, date_iso: str):
    tid = apisports_team_id(group, team_name)
    if not tid:
        return [], []
    inj = apisports_injuries(group, tid, date_iso)
    lineup = apisports_lineup(group, tid, date_iso)
    return inj, lineup

# ---------------------------------------------------------------------
# Built-in "pro" algorithm
# ---------------------------------------------------------------------
def edgefinder_predict(home_team, away_team, sport_key, event, scores_cache=None, elo_table=None, only_book=None):
    ...
    if scores_cache:
        ...
        eh = 1.0 / (1.0 + 10 ** ((Ra - (Rh + 30.0)) / 400.0))   # home expected win prob
        ea = 1.0 - eh
        gap = Rh - Ra
    ...
    # Injuries & lineups from APISPORTS
    ...
    # Injury/lineup adjustment (simple heuristic): +/- up to 5pp if one side lists >2 notable injuries or has starting XI posted
    inj_adj = 0.0
    if inj_home or inj_away:
        if len(inj_home) >= 3 and len(inj_away) < 2: inj_adj += 0.03 * -1  # home more banged up
        if len(inj_away) >= 3 and len(inj_home) < 2: inj_adj += 0.03       # away more banged up → home chance up
    if lineup_home and not lineup_away: inj_adj += 0.02  # known starters for home side
    if lineup_away and not lineup_home: inj_adj -= 0.02  # known starters for away only
...
# UI
# ---------------------------------------------------------------------
st.title("EdgeFinder — Pro (Single File)")

mode = st.radio("Mode", ["By sport", "By league"], horizontal=True)
...
# Fetch events
if "events" not in st.session_state or refresh:
    if mode == "By sport":
        evs = fetch_upcoming(regions=regions, markets=markets, odds_format=odds_format)
        st.session_state["events"] = [e for e in evs if group_from_key(e.get("sport_key","")) == group]
    else:
        key = name_to_key.get(league)
        st.session_state["events"] = fetch_league(key, regions=regions, markets=markets, odds_format=odds_format) if key else []

events = st.session_state.get("events", [])

if not events:
    st.info("No upcoming events for the current selection.")
else:
    ...
    if not filtered:
        ...
        # Preload scores for sport of first selected event
        scores_cache = []
        elo_table = {}
        if selected:
            sk_first = filtered[selected[0]].get("sport_key","")
